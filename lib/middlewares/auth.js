...
export default async ctx => {
  ...
  const currentUrl = process.server ? `http${isHTTPS(context.req) ? 's' : ''}://${context.req.headers.host + ctx.route.path}` : window.location.protocol + '//' + window.location.host + ctx.route.path
