...
module.exports = (options, req, res) => {
  ...
  return {
    ...
    async login(state, nonce) {
      await this.clear()
      await caches.request.set(state, nonce, options.login.expiry)
    },
    async validateState(state) {
      return caches.request.get(state)
    },
    async validateLogin(state, tokens) {
      const { nonce } = JWT.decode(tokens.access_token)
      const nonceStored = await caches.request.get(state)
      if (!nonce || nonce === nonceStored) {
    ...
    async tokensRequest(body) {
      return (await Fetch(options.endpoints.token_endpoint, {
        method: 'POST',
        body,
        headers: {
